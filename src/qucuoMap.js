...
export const QUCUO_MAP = {
  ...
  //      那地方也在成都西北，同样要求锦官城在东。
  // 订正为自西向东：曲措乡(0,0) → 雅江(2,0) → 锦官城(3,0)，出乡即东行。
  // 【为什么是 x=2 起步而不是 x=1】曲措乡自己的地界一直铺到 x=1（天都镇(1,1)、
  // 贡措海(1,2)）。头一版订正只查了「(1,0) 这格空不空」就把雅江放了进去，结果
  // 两座乡外大城插在乡的版图中间、雅江正卡在天都镇头顶上——出乡的门户该在边缘，
  // 不该在肚子里。退到 x=2 之后，中间空出来的 (1,0) 恰好就是雅江描述里
  // 「出西门行三日便是曲措乡界」的那三日路程。
  雅江: {
    x: 2, y: 0,
