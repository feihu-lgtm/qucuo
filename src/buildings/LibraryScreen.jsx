    ...
    <Overlay onClose={onClose} zoneTheme={zoneTheme} inline={inline}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {tab === "free" && FREE_BOOKS.map(book => (
          <div key={book.id} style={{ marginBottom: 10, padding: "10px 12px", background: "#10121a", borderRadius: 6, border: `1px solid ${zoneTheme.border}` }}>
            <div style={{ color: "#c8bfa0", fontSize: 13, marginBottom: 4 }}>{book.name}</div>
            <div style={{ color: "#7a7a6a", fontSize: 11, marginBottom: 6 }}>{book.desc}</div>
          ...
          return (
            <div key={book.id} style={{ marginBottom: 10, padding: "10px 12px", background: "#10121a", borderRadius: 6, border: `1px solid ${zoneTheme.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ color: QUALITY_COLOR[book.skillEntry?.quality] || "#c8bfa0", fontSize: 13 }}>{book.name}</span>
