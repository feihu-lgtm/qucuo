  ...
  return (
    <Overlay onClose={onClose} zoneTheme={zoneTheme} inline={inline}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {tab === "pledge" && (
          pawnable.length === 0
            ? <div style={{ color: "#5a5a4a" }}>背包里没有可质押的物品。</div>
            : pawnable.map(item => {
              const pledgeValue = Math.floor(item.sellPrice * 0.8);
              return (
                <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, padding: "8px 10px", background: "#10121a", borderRadius: 4, border: `1px solid ${zoneTheme.border}` }}>
                  <div>
                    <span style={{ color: QUALITY_COLOR[item.quality] || "#c8bfa0", fontSize: 12 }}>{item.name}</span>
                    <span style={{ color: "#5a5a4a", fontSize: 10, marginLeft: 6 }}>({item.quality})</span>
                  </div>
        ...
        {tab === "redeem" && (
          pledgedItems.length === 0
            ? <div style={{ color: "#5a5a4a" }}>没有质押中的物品。</div>
            : pledgedItems.map((p, i) => {
              const redeemCost = Math.floor(p.pledgeValue * 1.1);
              return (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, padding: "8px 10px", background: "#10121a", borderRadius: 4, border: `1px solid ${zoneTheme.border}` }}>
                  <div>
                    <span style={{ color: QUALITY_COLOR[p.item?.quality] || "#c8bfa0", fontSize: 12 }}>{p.item?.name}</span>
                    <span style={{ color: "#5a5a4a", fontSize: 10, marginLeft: 6 }}>质押了 {p.pledgeValue} 两</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
