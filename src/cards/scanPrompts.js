...
人设正文要**重写**，不能照抄原卡。原卡正文往往夹着小标题、外貌描写、与玩家的既有关系、说话示例、甚至 {{user}} 这类占位符——这些在本作里各有专门字段承接，混在一起注入给说书人就是一团乱麻。重写规矩：
- 120 字以内，一段连贯的话。不分行、不用小标题、不用方括号分节
- 第三人称客观叙述。原卡若用「你」称呼玩家，一律改掉
- 只写性格与行为倾向、与人相处的方式、当下的处境或目的
- 外貌不写（外貌锚点那项管）、与玩家的既有关系不写（好感初值那项管）、说话例句不写（初始态度那项管）
- 不保留 {{user}}、{{char}}、<user> 这类占位符，也不保留原卡的【】小标题
- 保真：性格、处境、目的照原文来，不要新编设定，不要拔高或贬低这个人
  ...
  "人物": [
    {
      ...
      "brief_why": "",
      "人设": "120字以内，按上面的重写规矩来。这是真正注入给说书人的那段",
      "levelCap": 2,
      "levelCap_why": "",
...
/**
 * 只在"导入当主角"时跑。
 * 【私密层 AI 抽取】原先这条线是硬性拒绝的（自动抽取可能为未成年角色生成身体
 * 描写）。现已改为默认开启，玩家在审改界面确认过风险后即生效。私密层只抽原文
 * 明确写到的项，没有依据的项一律留空，绝不编造。
...
【公开层】别人一眼能看见、以及动作描写要照着写的东西，共 7 项：
...
【私密层】贴身亲密时才会触碰到、衣物之下才看得见的，共 5 项：
- scars 疤痕印记：衣物遮住的伤疤、胎记、刺青，以及它们的来历
- scent 体味：凑近了才闻得到的气息
- intimate 身体细节：衣物之下的身体状态与尺寸细节
- sensitive 敏感处：被碰到会有明显反应的部位，以及反应是什么样
- habit 习惯癖好：亲密时的倾向、忌讳与下意识的小动作

每项 30 字以内，写具体可感的细节，不要写性格、不要写内心、不要写生平。
【私密层铁律】原文明确写到了某一项才填，原文一个字都没提的私密项一律留空字符串，
绝对不要靠推理或脑补去编造身体细节——留空后由玩家自己决定填什么。
...
输出格式：
{
  ...
  "bodyProfilePrivate": {"scars":"","scent":"","intimate":"","sensitive":"","habit":""},
  "special": {"根骨":5,"悟性":5,"体魄":5,"魅力":5,"智谋":5,"身法":5,"气运":5},
  "special_why": "",
  "persona": "120字以内的出身与来历，用第三人称写，供说书人参考。没有依据就留空",
  "缺失": ["原文完全没提到、留空了的 bodyProfile 与 bodyProfilePrivate 字段名"]
}`;
...
// 体貌字段清单。字段名与 bodyProfile.js 完全一致，顺序也照它。
// 公开层 7 项、私密层 5 项，供 cardScan 的清洗与审改界面共用，避免各写一份漂移。
export const BODY_PUBLIC_KEYS = ["height", "build", "face", "skin", "hair", "voice", "clothing"];
export const BODY_PRIVATE_KEYS = ["scars", "scent", "intimate", "sensitive", "habit"];
...
// 人设正文注入前的清洗上限。原生 preset.npcLore 里最长那条（梅朵）是 236 字，
// 所以 240 既容得下合规输出，也不至于让降级路径（AI 没出凝练版、只能拿原卡正文
// 顶上）把三千字整段灌进 prompt。
export const LORE_MAX = 240;

/**
 * 人设正文清洗。两条路都要过：
 *   AI 出的凝练版 —— 它多半已经干净，这里只做兜底（不听话时也不至于破坏格式）
 *   原卡正文顶上 —— AI 失败时的降级路径，这一步是刚需
 *
 * 【为什么必须压成单行】原生 npcLore 的格式是「首行 - 名字（身份）：正文」加
 * 若干条两格缩进的子行（外貌锚点／初见态度）。正文自己带换行会把这个结构冲散，
 * 说书人分不清一条到哪儿结束、下一条从哪儿开始。实测一张普通卡的正文里有六个
 * 换行加两级 markdown 标题，注入后整块结构全乱。
 *
 * 【为什么替换占位符而不是删掉】{{user}} 删掉会让句子缺主语（"替她挡了一句"
 * 变成"替她挡了一句"没了主语更糊）。换成玩家名，句子仍然通顺。
 */
export function sanitizeLore(raw, opts = {}) {
  const playerName = opts.playerName || "玩家";
  const charName = opts.charName || "此人";
  let t = String(raw || "");
  if (!t.trim()) return "";

  // 占位符。酒馆卡里这几种写法都常见，大小写不敏感
  t = t.replace(/\{\{\s*user\s*\}\}/gi, playerName)
       .replace(/<\s*user\s*>/gi, playerName)
       .replace(/\{\{\s*char\s*\}\}/gi, charName)
       .replace(/<\s*char\s*>/gi, charName);

  // 行首的 markdown 标题记号与方括号小标题。只剥行首——正文中间正常用到方括号
  // 的情况（引用书名、招式名）不该被动
  t = t.split(/\r?\n/)
       .map(line => line.replace(/^\s*#{1,6}\s*/, "").replace(/^\s*【[^】]{1,12}】\s*/, ""))
       .join(" ");

  // 压成单行
  t = t.replace(/\s+/g, " ").trim();
  return t.slice(0, LORE_MAX);
}
