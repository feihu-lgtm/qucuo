    ...
    <Overlay onClose={onClose} zoneTheme={zoneTheme} inline={inline}>
      ...
      <div style={{ padding: 16 }}>
        {!active.length && !completed.length && !locked.length && (
          <div style={{ color: "#7a7a6a", fontSize: 11 }}>尚无已接取的任务。与故事中的人物深入交谈，也许会有委托。</div>
        )}
            ...
            {active.map(q => {
              ...
              return (
                <div key={q.id} style={{ marginBottom: 10, padding: "10px 12px", background: "#10121a", borderRadius: 6, border: `1px solid ${zoneTheme.border}` }}>
                  ...
                  {q.giver && <div style={{ color: "#7a7a6a", fontSize: 10.5, marginBottom: 4 }}>委托人：{q.giver}</div>}
        ...
        {completed.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            ...
            {completed.map(q => {
              ...
              return (
                <div key={q.id} style={{ marginBottom: 8, padding: "8px 10px", background: negative ? "#140a0a" : "#0a140a", borderRadius: 4, border: `1px solid ${negative ? "#4a2a2a" : "#2a4a2a"}` }}>
        ...
        {locked.length > 0 && (
          <div>
            ...
            {locked.map(q => (
              <div key={q.id} style={{ marginBottom: 8, padding: "8px 10px", background: "#140a0a", borderRadius: 4, border: "1px solid #4a2a2a" }}>
