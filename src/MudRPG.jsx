...
export default function MudRPG({ initialLoadSlotId = null, initialOpenSettings = false } = {}) {
  ...
  const talkToNarrator = useCallback(async (content) => {
    ...
    try {
      ...
      const worldState = `[当前世界状态，仅供你了解背景，不必主动复述] 时间:${getTimeStr(time)} 房间:${room.name}${hasInnerMap(room.name) && innerRoomName ? `·${innerRoomName}` : ""}（${room.desc}） 房间里的人:${room.npcs.filter(n => isNpcVisibleInInnerRoom(room.name, innerRoomName, n)).map(n => n.name).join(",") || "无"} 玩家背包:${inv.join(",")}`;
