...
// time 字段一律为东八区（UTC+8）时间，手写字符串，组件不做任何时区换算
//
...
export const VERSION_HISTORY = [
  {
    codename: "教程任务「四宅家产」：集齐四把钥匙送土司礼服，三个触发点任一开任务、watcher 按持有数推进",
    time: "2026-08-02 10:30",
    notes: [
      "把「集齐四栋房子钥匙」变成一条自驱教程任务：任一触发点命中即开任务，进度跟着背包里实际持有的钥匙数走，集齐四把当场把一套土司礼服送进背包。",
      "①【三个触发点】端详溪边小屋钥匙（inspectItem 入口）、初次踏进溪边小屋（innerRoomName 变化）、跟旁白聊到溪边小屋——全都只做一件事：幂等置位 tutorial_estate_started，开不开、推进不推进由系统裁决，不靠 AI。",
      "②【系统裁决推进】置位后由 effect 播报一句「有趣的任务出现了」，建 COLLECT 任务（requiredCount=4，四阶段与四把钥匙一一对应）；推进按 ownedSafeHouseKeys 实时同步 currentStageIndex（1把→1/4 一路到 4把→完成），不设 completionFlag——钥匙获取顺序不固定（开局自带／赫连铸银灰钥匙／雪山派弟子别院令牌／锦官城衔尾蛇门环），按持有数驱动最稳。",
      "③【收官奖励】集齐即完成：橙档护甲「土司礼服」（七维各+1，藏锦金线、胸前四曲交汇纹），由 makeTusiRobe 现造入袋，不进百物录 catalog——独一份的任务奖励，避免 AI 在物件志里看见后随机发。完成与换装叙事走主叙事播报。",
      "验证：safeHouse.test.js 新增三组用例（ownedSafeHouseKeys 按背包计数／礼服橙档七维与独立实例／任务定义自洽且四阶段均无 completionFlag），vitest 全量复跑通过。",
    ],
  },
  {
    codename: "修复烹饪台/库房进不去：内层房间补挂 buildingId，8 个烹饪台/库房全部可开",
    time: "2026-08-02 00:13",
    notes: [
      "烹饪系统给灶房/库房造了 kitchen_/chest_ 建筑，但内层房间没挂 buildingId，左栏在内层只显示 rooms[房间].buildingId 那一个，导致站在灶房/起居室看不到建筑、打不开。",
      "①【溪边小屋】起居室补 chest_xibian、灶房补 kitchen_xibian——小屋内的烹饪台与库房从此可开。",
      "②【其余三栋】山间别墅／弟子别院／蜀王庄的灶房与库房房间同步补挂 buildingId，两处修复合计让 8 个烹饪台/库房全部可开。",
      "验证：四栋小屋的内层房间逐一核对 buildingId，vitest 全量通过。",
    ],
  },
  {
    codename: "烹饪系统（六格烹饪台+AI菜名小总结+库房预填食材）＋酒馆经营沙盒接入开始界面",
    time: "2026-08-01 08:48",
    notes: [
      "灶房真正能开火了：六格槽位烹饪台（料×4＋技法＋炊具）落进四处灶房，5 技法／10 炊具／10 配方，自由组合还有「妙手偶得」——数值归系统、菜名归 AI。",
      "①【数据与裁决】cooking.js 纯数据＋纯函数：技法五种（炖/炒/烤/腌/蒸，蒸需带蒸笼的炊具）、炊具十件（各带 hpBonus/turnsBonus，溪边旧铁锅灶房自带永远可用）、配方十道（料槽组合不分先后、排序归一化比对，回血＋buff）。cookDesign 把命中料的 INGREDIENT_LORE 风物志拼进 prompt，成菜由主叙事 AI 起名、写说书人风味小总结，想象有据不凭空。",
      "②【库房独立成建筑】木箱从右上角家园面板挪成左栏建筑 BUILDING_TYPE.CHEST，站在据点就能进库房；四栋小屋库房首次打开预填全套食材＋调味料（白/绿常见件×3、蓝稀罕件×2），落地就能开火。",
      "③【酒馆经营沙盒】tavernSim 独立沙盒（simEngine 裁决＋simNarrator 说书＋simData 数据，自带独立 localStorage 存档）接入开始界面，主游戏之外单独可玩。",
      "验证：文件树登记至 230；cooking/simEngine 新增测试与全量 777 全过，verify 构建通过。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "接着上一版把入册打磨顺手：选主角、批量、配装备、手感四件。",
      "①【从扫出的人里选主角】群像卡（比如美食四天王一张卡七人）主角混在众人里。「我自己」页签顶部加一排选择器，点谁谁就当主角——其名字/七维/内外功/人设填进主角档案，并自动从「入游戏」名单挪走，不重复入册。选「空白·自己填」则不从卡里取、主角从头自己填。",
      "②【名单批量设落脚/品阶】一张卡的人常同属一个据点、或该整体拔高一档，逐个点开太费手。名单顶部加一条批量：选好据点后一键「全驻此地／全游走／全不落地」，品阶「白绿蓝紫橙红」一键调档，只作用于勾选中、且不是主角的人。招式路数因人而异没法一键套，仍在右栏逐个点开定。",
      "③【AI 一键配装备】类比一键规划落脚，新增 carryPlan.js：AI 按每人身份/营生/品阶配 2-4 件随身物（武人配兵器、医者配药囊、僧道配念珠……），回来的物件过 sanitizeCarryPlan 校验（品阶不超此人、类别合法、一人至多五件）后填进 carry，玩家还能自己改。物件当自造物品产出，运行时同名会自动查百物录补数值。按批调用，与落脚共用令牌桶与那条进度条。",
      "④【新控件加 hover 手感】入册这几处新加的可点控件（选主角、批量按钮）原来是静态内联样式、鼠标移上去没反馈。抽了个 TapSpan：hover 时描边提亮并上浮 1px、过渡 .14s，选中态描边与字都用高亮色。项目其余按钮维持原样，只给这几处新交互加，不动全局风格。",
      "验证：vitest 733/733、pages build 通过。配装/批量/选主角的实际效果依赖 AI 调用与角色卡，需在真实环境走一遍验收。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      ...
      "⑩【补记一次事故·我在测试红的状态下推了一次】写完这条版本记录后跑验证，用的是 「vitest run 管道接 grep Tests」 判断成败——grep 只要匹配到那一行就退出 0，而那行内容其实是「3 failed」。&& 链照样往下走，于是 build、commit、push 全跑完了。根因是这条记录本身：第④条里写了一对英文双引号，而整条 notes 就是双引号包裹的字符串，提前闭合导致 version.js 语法坏掉、docsTree 那三条 version 守卫全红。文件头注释只警告过反引号，双引号同样致命，已改用中文引号。两个教训：一是判断命令成败要用退出码，不能用 grep 匹配到某行文字；二是这个项目栽过「version.js 写坏引号、测试全绿构建失败」，这次是它的变体——守卫这次抓住了，是我没看守卫的结论。已用 if 退出码重验：vitest 与 pages build 均通过后才补推。",
  ...
  },
  {
    ...
    notes: [
      "看实测截图改的。上几轮把面板从 1120×800 改成撑满全屏，收益是落脚那一节终于翻得到了，代价是一批按窄栏定的比例全都崩了，这轮结账。",
      "①【根因·横条素材被横向拉近 8 倍】bar_wood 458×120、bar_paper 425×132、bar_paper2 441×134，三张的结构都是两端各一块约 95px 宽的金色包角装饰、上下各约 15px 金边、只有中段的木纹或纸面可以拉伸。代码里一律 backgroundSize 100% 100% 整幅强拉。面板还锁 1120 宽的时候 Bar 实际宽 230 到 890px、拉 1.5 到 3 倍，观感过得去——version.js 里那条注释的原话就是「那批图本就是整幅设计好的，拉伸幅度不大时观感没问题」，限定语是「拉伸幅度不大时」。全屏之后左栏就有 1000px 宽而高度仍是 34px，比例从原图的 3.8 变成 29，横向拉了将近 8 倍：两端装饰件糊成一片、上下金边压成细线、标题文字压在装饰件上，底栏那条更狠，拉伸后的装饰件把「回名单」按钮整个盖住。",
      "②【改法·border-image】新增 ReviewParts 的 barFrame(img, slice=15, side=95)：四角保持原始像素、四边只沿一个方向拉伸、中段 fill 双向填充。三处横条（Bar 组件、顶部标题栏、底栏）全部改用它。连带好处是 border 占掉左右各 95px 的盒模型空间，文字自然落在中段木纹上，不必再靠 padding 去躲装饰件——原来 padding 只有 16px，根本躲不开。Bar 的 minHeight 从 34 提到 46（上下 border 吃 30px，留 16px 给 13px 的字）。",
      "③【面板底纹的三角边界】skin_wusha 是 512×512 的方图，配 backgroundSize cover 会被放大到铺满全屏面板，纹理里那条对角线于是被放大成一道横贯半屏的三角形暗块——截图里左栏中段那片诡异的斜向阴影就是它，不是渲染 bug。改成按原尺寸 512px 平铺，纹理密度回归正常。",
      "④【左栏名单居中·按要求】一张卡常见三五个人，顶部对齐会在下面留一大片死黑。改成 margin auto 0 垂直居中，外层 alignSelf center 加 maxWidth 380 水平限宽——4:6 分栏后左栏上千像素宽，条目不限宽会拉得很散。用 margin auto 而不是 justifyContent center：后者在内容超出容器时会把顶部裁掉且滚不到，是 flex 的老陷阱。",
      "⑤【顺带修一个真 bug·同一个人两处说法相反】截图里苏婉仪的名单条目显示「游·0处」，而右侧落脚却高亮「不落地」。根因是名单直接读 n.placement.mode（原始值 wander），而那组 Pills 读的是 normalizePlacement 之后的值——游走但一个据点权重都没给属于无效配置，归一化会退回 mention。两处口径不一致，玩家会以为这人会游走，实际压根不会出现。名单也改走归一化后的值。",
      "⑥【AI 荐位按钮不再顶到天边】它跟三个模式钮之间隔着一个 flex 1 的 spacer，4:6 分栏后右栏上千像素宽，那按钮就被顶成一个跟谁都不相干的孤零零元素。去掉 spacer 紧跟模式钮。",
      "⑦【这轮的视觉改动我没法自验】这个环境装不了 playwright 的浏览器内核，仓库里那个 ui-shot.mjs 跑不起来。9-slice 的切片值 15/95 是照素材目测定的（458 宽的两端装饰件约占 20%），比例对但可能差几个像素——若装饰件边缘出现细线或被切掉一点，调 barFrame 的第二三个参数即可，那两个数就是为了好调才提成参数的。",
      "⑧【新增 7 条守卫】接在 cardImportLayout.test.js 后面：barFrame 产出的是 border-image 不是 backgroundSize、Bar 用 barFrame 且不含 100% 100%、顶栏底栏各有一处 barFrame、Bar 的 minHeight 容得下上下 border、底纹不用 cover、名单用 margin auto 0、名单落脚标记走 normalizePlacement。",
      "验证：vitest 704/704（新增 7 条）；pages build 通过；propsCheck 1471 属性全解析。视觉效果需实测确认。",
    ],
  },
  {
    codename: "人设正文改由 AI 按本作格式重写，原卡全文留在审改界面对照；世界观条目不再被当成人",
    time: "2026-07-30 14:40",
    notes: [
      "查 trace 查出来的：入册人设注入进 prompt 的样子跟本作原生格式完全不是一回事，说书人拿到的是一团乱麻。",
      "①【根因·stage2 的 schema 里压根没有 entry 这一项】它输出 brief／外貌锚点／初始态度／数值／招式／里程碑，但 entry 一直是 p.entry——**卡的原始正文直传**，normalizeStage2 与 fallbackNpc 两处都是。所以「把外部角色卡换算成游戏内 NPC 数据」这件事只完成了数值那一半，人设正文根本没换算。registerImported 里也是 entry: n.entry || 空串，无清洗无截断。",
      "②【实测证据】拿一张 191 字的普通卡跑，注入出来是这样：首行「- 柳青梧（寻访旧案的青衫剑客）：## 基本信息」，接着六个换行分出「姓名：柳青梧　年龄：二十三」「【外貌】…」「【背景】…」「【与占位符的关系】你替她挡了一句难听的话」「【说话风格】例句：「不必。」」「【NSFW】略。」，最后才是两格缩进的外貌锚点。七宗罪：markdown 标题混入、方括号小标题与本作的缩进子项格式打架、占位符原样注入（说书人看到字面的两层花括号 user）、第二人称（原生一律第三人称客观）、外貌描写重复两遍（卡正文的外貌段＋本作的外貌锚点）、NSFW 残留垃圾照抄、以及最要紧的——首行内部的换行把「首行加缩进子行」这个结构冲散，说书人分不清一条到哪结束。",
      "③【格式基准对照】原生 preset.npcLore 六条实测 30 到 236 字，一段连贯的第三人称描写，只写性格与行为，外貌单独走两格缩进的外貌锚点子行。入册那张温和的卡就已经 397 字（含结构垃圾），而中文卡池里正文上千字很常见。",
      "④【改法·stage2 加「人设」输出字段】要求 AI 重写而不是照抄：120 字以内一段连贯的话、不分行不用小标题、第三人称、只写性格与行为倾向与处境目的、外貌不写（外貌锚点那项管）、与玩家的既有关系不写（好感初值那项管）、说话例句不写（初始态度那项管）、不保留占位符与方括号标题、保真不新编设定。",
      "⑤【新增 sanitizeLore 兜底】两条路都过：AI 出的凝练版多半已干净，这一步防它不听话；AI 整批失败时拿原卡正文顶上，这一步是刚需。做四件事——占位符换成玩家名／角色名（换而不删，删掉句子会缺主语）、剥行首的 markdown 记号与方括号小标题（只剥行首，正文中间的【无名剑法】这类不动）、压成单行、截到 LORE_MAX 240（原生最长那条梅朵 236 字，240 既容得下合规输出又不至于让降级路径把三千字灌进去）。代码清洗不掉第二人称，那是 AI 重写的职责，降级路径下 UI 会标出来提示自己改。",
      "⑥【原卡全文留着对照·按要求】新增 rawEntry 字段存原文一字不动，进 IndexedDB 但不进任何 prompt。审改界面人设那一节：上面是可编辑的凝练版（带字数计数与来源标记，AI 出的标 AI、降级顶上的标默认），下面并列摊开原卡全文（限高 min(26vh,280px) 滚动、可选中复制）。默认摊开不折叠——它存在的唯一理由就是让人判断重写得准不准，折起来就没人看，那留这份数据也没意义。",
      "⑦【另一处独立问题·世界观条目被声明成人物】getImportedNpcLore 把地理／势力／规矩／物件条目拼成「- 标签：内容」混进同一个数组，而 buildNpcLoreBlock 的两个外壳文案是专门针对人写的。实测确认：玩家提一句锦官城，说书人就读到「【仅被提及、并不在场的人物设定】（…绝不能让他们凭空出现在这个场景、开口说话…）－ 锦官城：西南第一大城」。importedRegistry 的注释解释过「机制通用不必为地理另造一套」——触发机制通用这个判断是对的，漏的是外壳文案那一层。改法：world 条目打 isWorld 标记，matchNpcLore 传递它，buildNpcLoreBlock 分出第三段「【相关地理与势力】（这些是本轮提到的地点、门派、规矩或物件，不是人，不会说话也不会登场）」。原生 preset 条目没有这个标记，undefined 走人物段，行为不变。",
      "⑧【我这轮又栽了一次同样的坑】改 worldbook 时三处替换写在一个脚本里、写文件放在最后，第三个锚点对不上就抛异常退出，于是前两处替换只存在于内存里、文件根本没动。跟前几轮那个「不带 assert 的字符串替换静默空转」是同一族的错——这次有 assert 抓住了，但顺序不对：assert 全部通过之后再一次性写盘才对，不能边改边攒最后统一落盘却让 assert 中途退出。",
      "⑨【新增 cards/loreFormat.test.js 十二条】清洗六条（占位符换名不删、剥行首标题、正文中间方括号不动、压单行、上限容得下原生最长条目、空输入不抛错）；stage2 规范两条（schema 有人设字段、重写规范把四件事说清）；世界观分块四条（地理条目落进地理段且不出现在人物段、人物照旧、原生条目行为不变、只有地理命中时不生出空的人物段）。",
      "验证：vitest 697/697（新增 12 条）；pages build 通过；propsCheck 1471 属性全解析；placementTest 通过；文件树 218→219、32→33 份测试。改完的注入文本实测与原生格式逐行对齐（首行 - 名字（身份）：正文、两格缩进的外貌锚点与初见态度），entry 87 字入 prompt、rawEntry 191 字只给玩家看。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "撑满全屏之后原来那套按 1120 宽定的比例全都不合用了，三处一起改。",
      "①【随身物选择器静默截断 406 件到 60 件】CarryPicker 的候选列表末尾有一个 slice(0,60)，且没有任何提示。百物录现在 406 件（兵器 101 / 护具 59 / 饰物 82 / 杂物 164），玩家不搜名字就永远翻不到后面那 346 件，看起来像是库里只有这些。这类 bug 最难发现——不报错、不崩溃，只是让人以为东西就这么点。去掉截断，406 个条目对 React 不算负担。",
      "②【放开之后真正需要的是筛得动】原来只有类别加名字两个维度。补上品阶筛选（406 件里按品阶挑很常用：白 92 / 绿 97 / 蓝 95 / 紫 61 / 橙 40 / 红 21），关键词连 desc 一起搜（找「止血的」比记得住药名实用），列表上方加计数条常显「百物录 406 件 · 当前 N 件 · 已选 M」，容器高度从 128px 提到 min(38vh,380px)——原来 128px 只看得见五六行，配 60 件上限，可用性等于没有。",
      "③【4:6 分栏】左栏原来定宽 230px，是面板锁在 1120 宽时定的。撑满之后名单挤在一条窄带里，运行日志横向被压得每行都折。改成 flex 4 比 6。两栏都必须带 minWidth 0——flex 子项默认 min-width auto，长人名会把栏撑破这个比例，这一条不加改了等于没改。",
      "④【日志高度用 vh 不用定值】从 118px 拉到 min(30vh,320px)。不用定值是因为左栏是纵向 flex，名单区 flex 1、日志固定高，给死 320 的话在矮窗口下会把上面的人物名单挤到看不见——拉长这个需求本身就更需要按视口给。",
      "⑤【顺带利用变宽的左栏】落脚标记原来只有一个字，驻或游。宽度够了改成写全：驻·天都镇 / 游·3处，未选据点的驻场显示「驻·未选据点」，一眼能看出哪几个人的配置是无效的（没选据点的驻场会被 normalizePlacement 退回不落地）。",
      "⑥【新增 cardImportLayout.test.js 守这三条】它们全都属于「不报错、不崩溃、只是玩家看不到东西」那一类，跑测试跑不出来，只能盯源码指纹——照 centerPanelLayout.test.js 的先例做源码级守卫。七条断言：百物录规模够大、候选列表没有 slice 硬截断、三个筛选维度都在、计数条存在、4:6 且两栏带 minWidth 0、panel 没被写回 1120×800、Terminal 高度用 vh。每条都写了「指纹可能变了请同步这条守卫」的提示。",
      "⑦【顺带记一笔·npc 驻地本来就能选】上一轮误以为落脚设定缺 UI。实际 Placement 组件（据点下拉、内层房下拉、权重 chip、时辰）早就写完并接进第 15 节了，缺的只是 AI 草稿。当时看不见的真正原因是①的同源问题：面板没撑满、横向一窄、据点 chip 疯狂换行把整节推到滚动区底部。功能在，是布局把它埋了。",
      "验证：vitest 673/673（新增 7 条）；pages build 通过；propsCheck 1462 属性全解析；文件树 215→216、30→31 份测试。",
    ],
  },
  {
    codename: "修 AI 荐位报「不是合法 JSON」：写死的 maxTokens 700 被思考 token 吃穿",
    time: "2026-07-30 11:20",
    notes: [
      "现象：点 AI 荐位报「模型返回的不是合法 JSON：Unexpected token 反引号」。错误文案把方向指反了——不是模型不听话，是输出被截断在了开头。",
      "①【三条证据同时成立只有一个解释】parseJsonLoose 有三层救援：原文直接 parse、剥围栏、截最外层括号。报错说第一层撞在反引号上，说明原文以围栏开头；第二层没生效，说明闭围栏不存在；第三层也没生效，说明右方括号也不存在。三者同时成立，就是输出在写完开围栏加一个左方括号之后被切断了。",
      "②【根因是我上一轮写死了 maxTokens】落脚规划单人给 700、批量给 1600，按正文实际长度算的——一个人的规划正文只有六七十 token，看着绰绰有余。但带思考的模型里思考 token 与正文共用这一份额度，700 在思考阶段就被吃穿。对照 cardScan.js 的 STAGE_MAX_TOKENS 是 1:6000 / 2:4500 / 3:1500 / 4:4000，全都远大于正文所需，就是为这个留的余量。",
      "③【这个坑项目自己记过，我是漏网的第 11 处】apiConfig.js 的 DEFAULT_CALL_TOKEN_LIMITS 里那条注释写着：narratorWhisper 原本写死 600，是上一轮「9 处写死抽成配置」漏网的第 10 处，带思考的模型下 600 被吃穿后正文半句就撞 length 截断。症状一模一样。改法：placementPlan.js 导出 PLAN_MAX_TOKENS（single 3000 / batch 6000），两处调用点改用命名常量。",
      "④【顺带修掉同类隐患一处】ReviewNpc 的 askAiDesc（让 AI 写自造物品的描述）写死 400，我上一轮就是照抄它才写出 700。它的症状比落脚规划更隐蔽：要的是纯文本，截断不报错，只是玩家看到的描述莫名缺半截，还以为模型没写好。改成 ITEM_DESC_MAX_TOKENS 2000。",
      "⑤【parseJsonLoose 补第四层救援·独立于本次功能】被截断的数组，找到最后一个刚闭合的顶层对象、丢掉后面残缺的半个、补上右括号，至少救回前面几条。批量一次六人，截断通常发生在最后一两人身上，救回来意味着前四五人的规划还在——跟 cardScan 那边「一批失败不该让前面白丢」同一个道理。四个扫描阶段都受益。",
      "⑥【不能用 lastIndexOf 找边界】条目里有嵌套对象（落脚规划的 weights 就是），最后一个右花括号往往属于嵌套那层。形如 逗号 name 乙 weights 锦官 这样断掉的输入，lastIndexOf 会截到错误位置，补出来的还是坏 JSON。得自己扫一遍做括号计数，且字符串内的括号不能参与计数（why 字段里就可能出现花括号）。",
      "⑦【救援必须排在「截最外层括号」之前，这条是写测试才发现的】那一步是花括号优先于方括号，对 一个完整对象加一个残缺对象 这种输入，它会先截出第一个完整对象并成功解析成**单个对象**。调用方拿到的就不是数组了，sanitizePlacementPlan 收到非数组直接返回空，救援等于白做。三条用例全挂在这个顺序上。但也不能无脑提前：有的阶段返回的本来就是单个对象，对象里若嵌着数组，提前会救出一个丢了外层键的数组片段，形状对不上比失败更糟。所以加 expectsArray 判断，只在正文本身以左方括号开头时提前。",
      "⑧【TRUNCATED 的判定原来漏了方括号】原本只看 indexOf 左花括号，而截断可能早到只吐出围栏加一个左方括号——那时找不到花括号，会被误判成 NOT_JSON 走熔断，提示玩家「模型压根没在输出 JSON」。两者的补救办法正好相反：截断该调大额度，NOT_JSON 该改提示词并停止烧额度。改成两种括号都看，并把文案写成「输出被截断（写到第 N 字断掉），把 maxTokens 调大或拆小批次」。",
      "⑨【报错带上模型实际吐的内容】两个入口的 catch 原来只显示 message 前 30-40 字，「不是合法 JSON」和「被截断」看起来一样。现在附上 e.raw 的开头五十字，一眼能分辨是截断、是闲聊、还是格式跑偏。",
      "⑩【还有三处写死小值没动】qucuoMap.js 第 213 行 maxTokens 10、knowledge.js 两处 120。它们不是本次改动引入的，用途也没查清（10 这种量级在思考模型下必被吃穿），扩大改动面不如留给下一轮单独核。battleNarration.js 的 800 带注释说明已考虑过思考占用，不动。",
      "验证：vitest 666/666（新增 9 条截断救援用例：只有开围栏、嵌套对象里截断、字符串内括号不计数、一条都救不回时抛 TRUNCATED 而非 NOT_JSON、该熔断的仍熔断）；pages build 通过；propsCheck 1461 属性全解析；placementTest.mjs 数据层回归通过。",
    ],
  },
  {
    codename: "入册面板改真全屏 + AI 荐位（落脚一键规划）",
    time: "2026-07-30 10:30",
    notes: [
      "入册界面三件事：面板撑满、落脚处能让 AI 出草稿、私密层扫描的请求被拒。",
      "①【全屏是真 bug，且连带埋掉了落脚设定】panel 写死 width min(1120px,95vw) / height min(800px,93vh)，1600 宽以上的屏幕四边留一圈空遮罩。更要紧的是审改页右侧是 16 个 Section 的长表单，落脚排在第 15 节，横向被压窄时那二十几个据点 chip 疯狂换行、把内容一路往下推，结果玩家根本翻不到「落脚 · 他会出现在哪」——于是看起来像是没做这个功能。撑满之后左栏 230px 不动，多出来的宽度全给表单。代价是点遮罩关闭失效（没有遮罩了），头部 btn_close 仍在。",
      "②【落脚编辑器本来就在，缺的是 AI 草稿】Placement 组件（据点下拉、内层房下拉、权重 chip 点一下加二十五、时辰两个数字框）和数据层（normalizePlacement / getImportedForDistrict / getImportedResidentNames 幽灵过滤）此前都已写完。这次新增的是 src/cards/placementPlan.js：AI 按人设判断该 mention / resident / wander，出据点或权重，玩家再改。",
      "③【为什么不塞进 scanPrompts.js】那个文件装的是导入流水线的四个阶段，有固定顺序、由 cardScan 串起来跑。荐位发生在审改期，玩家点按钮才触发，可以跑零次也可以反复跑，混进去会让「阶段」这个概念失去意义。",
      "④【AI 在这里只被允许做一件事】据点名一律从 PLANNABLE_DISTRICTS 里挑（QUCUO_MAP 全部键去掉心灵之海与第三新东京市，共 16 个），清单外的名字直接丢弃退回不落地。这条不是防模型作恶，是防它把「天都镇」写成「天都城」——那种错误在 getImportedForDistrict 里的表现是角色永远不出现，且没有任何报错。同理权重吸附到 25/50/75/100 四档（模型给 30、给 80 都常见，直接用会让档位失去意义），时辰夹到 0-23、起止颠倒自动调正。",
      "⑤【prompt 有真凭据可依，不靠名字猜】QUCUO_MAP 每个据点都带 desc（16 个共 831 字，整份塞得进去），配上 DISTRICT_REGION 的地域分类（村镇/官城/市镇/宗教/门派/山野/异域），模型判断「打铁的该驻天都镇、游方僧该在喇嘛庙一带游走」有依据。返回里要求带一句 why 并显示在撕纸条上，玩家能看出它有没有瞎猜。",
      "⑥【批量按六人一批，一批失败不中断】令牌桶是 5 次/分钟。一人一次的话十个人要等两分钟，而落脚判断只需要身份与来历，六个人的摘要塞进一个 prompt 完全放得下——三十人也只烧六次额度。每批独立调用，第二批 429 不该让第一批已经规划好的人白丢；失败批次记数，末尾一并告诉玩家哪几批没成、可以再点一次。sanitizePlacementPlan 返回的 index 是相对本批的，分批时把全局下标一起带上，否则会改错人。",
      "⑦【不让 AI 选内层房间】内层房要先知道落哪个据点才能列，一次批量里每人据点都不同，把全部据点的房间清单都塞进 prompt 会让它膨胀好几倍，而这一项玩家在现成下拉里点一下就有。AI 出据点，人补房间。",
      "⑧【私密层 AI 扫描：拒绝，理由是项目自己写过三遍的】提出过给私密五项接 AI 扫描。这条线在四个层次上是一致的：bodyProfile.js 的 PUBLIC/INTIMATE 分层、ReviewPlayer.jsx 头部两条硬规矩的第一条、UI 里那个 warn Note、以及 gateBodyProfile 里私密层只认 nsfw 一个布尔（公开层有 PUBLIC_KEYS 关键词通道，私密层没有）。原话是「让导入器自动从卡里抽身体细节／敏感处等于批量给未成年角色生成身体描写，这不是可配置项」。入册器的输入是任意外部卡，扫描器判断不了卡里那个人多大；手填是一次一个、填的人知道在填谁，AI 抽取是一次一批、对未知年龄的角色自动生成。功能上也没被卡住：bodyProfilePrivate 五项手填通道一直开着，MudRPG 两处合并与 gateBodyProfile 的门控全都通着。",
      "⑨【文件树守卫连带更新】新增两个文件触发 docsTree 的两条断言（每个源文件都登记在册、声明文件数与实际一致），规模行 213→215、29→30 份测试，cards 段与测试清单各补一行。这条守卫这次起作用了。",
      "验证：vitest 657/657（新增 placementPlan.test.js 14 条：编造地名退回并标 rejected、权重吸附与清单外剔除、时辰夹取与颠倒调正、别名对人、重复条目丢弃、非数组不抛错、why 截断）；vite build --config vite.config.pages.js 通过；propsCheck 1461 个 JSX 属性全解析（比上轮多 3 个，正是新加的 props，这次改动恰好属于它专抓的那一类）；scripts/placementTest.mjs 数据层回归通过。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "①【根因是时序，不是分类不准】原来的顺序是「机器初判 → 玩家在勾选界面定名单 → 点扫描才跑 AI 分类」。而 buildPersonGroups 末尾有一道按勾选名单的过滤，于是 AI 认出「血角」「三霄」是人也进不了阶段二，白认一场。8 张卡实测落在「待定」的真人物：血角/岩蹄/铁角（断法大陆）、金灵圣母/三霄（封神异闻录）、俞宝儿/樊长玉/谢征（逐玉），全是标签与关键词一模一样、机器判据（要求 label !== keys[0] 且 label 含 keys[0]）够不着的那种。",
      "②【新增 runClassify，只跑阶段一】拖卡后多一个「先认人」按钮，结果写进同一个缓存槽（stage 1），所以之后点「扫一遍」不会重复调用，总预算不变。认完名单整个换成 AI 那份，不再分「候选/待定」两栏；认出来的默认全勾上，玩家可逐个取消——最终否决权不交给 AI。实测封神异闻录认人前 1 人＋3 条待定，认人后 4 人，金灵圣母与三霄都进来了，三霄还标了「合」（AI 认出那几条写的是同一个人）。",
      "③【阶段一从发摘要改成发正文】原先每条只发前 120 字，是为省额度。但反代不计费，省下的那点 token 换来的是判错——「三霄」「血角」这类条目看头 120 字根本分不出是人是概念，而这恰恰是最需要 AI 帮忙的一类。",
      "④【为什么不干脆发全文】不是为省钱，是两个硬限制：反代对单次请求体大小常有上限，以及实测有单条 27294 字、整卡 133095 字的世界书（哀鸿城破与万象枢机）。发全文会让一次分类等上几十秒、还可能被网关直接拒。改成单条上限 3000 字——一条人物设定的前 3000 字必然已经写清「这是谁」，分类要的信息全在开头。另加整卡 90000 字的总量兜底，超了按条目均摊而不是先到先得：先到先得会让排在前面的巨型条目吃掉全部配额，后面的人物条目一个字都发不出去。",
      "⑤【实测输入量】封神异闻录 4507→8549 字、断法大陆 13099→21452、逐玉 6965→11982、春明子 2435→3886、哀鸿城破 7045→33215（它那条 2.7 万字的原文被截到 3000）。最大 33KB，Gemini 上下文与反代请求体都吃得下。阶段一的输出上限跟着从 4000 提到 6000 token。",
      "⑥【机器初判仍保持保守，没放开】既然 AI 会覆盖它，看着可以把 KIND_RULES 判宽些。但它是 AI 失败时的兜底——免费反代随时 429，兜底宁可少认几个人，也不要把「人间共主」「混血儿·处境与能力规律」当活人写进叙事。",
      "验证：esbuild 过；scripts/classifyTest.mjs 用 mock 跑通认人链路（原本落待定的金灵圣母与三霄进名单、同人分组标记、entry 随名单带出、二次认人 calls=0 走缓存）。vitest 全量仍需 npm run verify。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "五项界面反馈一并做了，另外把审改那一段按「导入映射表」的顺序铺成全字段。",
      "①【七维三种改法】原来只有滑条，想把 5 调成 6 得在 30px 宽的条上找准 10% 的位置。现在滑条＋加减号＋直接键入都留着，加减号用 stones 的 btn_blank 做底，到 0 或 10 自动置灰。内外功（0-100）复用同一个 StatRow，只是量程不同。",
      "②【输入框文字居中】单行框一律居中，装的都是名字/身份/数值这类短内容；多行框保持左对齐——外貌锚点、出身来历那种成段文字居中会读不下去。TextField 留了 align prop 可强制。",
      "③【左下角换成终端】视觉照 TraceViewer 抄（#161510 底、#9a9a8a 字、#8ac8b8 高亮、等宽、pre-wrap），带时间戳与符号分级（▸开始 ✓成功 ✕失败 ↻重试 ⧗等额度）。日志提到主组件累积，所以扫描时与审改时看的是同一份，右上角可放大成 760px 浮层。替掉了原来那行「实调 N 次」——那只有结果没有过程，而 5 次/分钟的额度下最需要看见的是「现在卡在哪、为什么等、哪一批降级了」。",
      "④【品阶改下拉＋石框】原来点一下循环换档，从白调到红要点五次，而且看不见一共有哪几档。现在玉石图只管显示，选档交给 select，外面用 box_wood 素材包一层壳，appearance:none 把原生控件藏进去，选项文字带各档玉色。",
      "⑤【全字段审改，拆成三个文件】CardImportScreen 只留外壳与分派，NPC 的十六项在 ReviewNpc.jsx、主角的十二项在 ReviewPlayer.jsx，共用零件在 ReviewParts.jsx。开头就选「众人／我自己」，不再是藏在勾选框里。",
      "⑥【招式编辑器走原型系统，不是勾标志位】本来打算让玩家逐个调 energyCost/baseDamageMultiplier 那十几个标志位，读完 combat/moveArchetypes.js 才发现方向错了：本作的招式效果由 原型 × 品阶 自动解出——同一个「防守反击」，白档只是挡一下，红档是听桥（近乎全免＋满反打＋抢先结算＋失手重罚）。所以玩家该决定的是「这人走哪条路数」，改品阶招式会整套跟着变。编辑器给的是七个原型的下拉＋起名＋一句话描述，底下「数值」按钮把 resolveArchetype 解出来的实际效果摊开显示（英文键全部译成人话），不用猜。回气槽固定走「回气」原型只能改名，疗伤是可选第五招。",
      "⑦【新增可定制字段】内功／外功（0-100，按品阶给基准值，气血与出手威力由它们算出、只读显示）、随身物（从 CATALOG 按类别筛选＋搜名字，留空则运行时走 rollNpcCarry 四池随机）、立绘（借用现有十张）、性别、体貌私密层五项（只手填）、说话风格示范（取卡的 mes_example 进 9 号位，那位置一直空着）、世界观条目逐条取舍。",
      "⑧【读取侧第四个接入点】deriveSignatureMoveset 原本只查 NPC_SIGNATURE_MOVES[名字]，现在先查入册表——入册的优先，那是玩家亲手配的。同时 toPoolLike 补带 neigong/waigong/carry/portrait，否则玩家调的内外功传不到 applyNpcDefaults，气血与攻击还是按品阶默认值算，等于白调。",
      "⑨【世界观条目跟人物走同一条路】地理／势力／规矩条目同样是「有关键词、被提到就注入」，matchNpcLore 的三源触发对它们一样适用。虽然那函数叫 npcLore，机制其实是通用的关键词点灯，不必为地理另造一套。",
      "⑩【卡里被丢弃的东西列出来给人看】正则脚本、酒馆助手界面、第三方扩展、框架类条目（状态栏／格式／思维链），在 NPC 页底部单列一段标「已丢」，附上丢弃理由。别让它们悄悄消失——玩家至少该知道这张卡里有什么没进来。",
      "验证：esbuild 逐个过了 8 个改动文件，并做了一次 bundle 级检查（560kb，全部 import 可解析、无循环依赖）。docsTree 三条文件系统守卫本地模拟全绿（登记 0 缺、幽灵 0 条、声明 213 = 实际 213；统计行的测试份数与行数也按实测更新为 29 份 / 4.71 万行）。vitest 全量仍需 CI 确认。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "①【根因·我只核了自己知道的那两条】上一条推之前我手动模拟了 docsTree.test.js 的「每个源文件都登记在册」与「没有幽灵条目」，两条都过，就在 commit message 与 version 记录里写了「docsTree 两条守卫已按规矩照做」。实际那个 describe 块里有三条，第三条是「文档里声明的文件数与实际一致（防统计行过期）」——它拿正则从文件树头部抓「N 个 js/jsx 文件」这句话，跟真实文件数比。我加了 7 个文件，声明还是 203，CI 就红在这一条。643 条测试 642 过 1 红。",
      "②【教训不是「忘了改数字」而是「凭印象核测试」】我没读完整个测试文件就断言核过了，这跟前几轮踩的坑同源：只看了局部就推全量。这次改法是把 docsTree.test.js 从头到尾读完（12 条），逐条确认哪些会被我的改动影响——除了统计行那条，另外两条本来该担心的（源码不许硬编码 public 绝对路径、MudRPG 不许有 TDZ 风险）CI 显示都过了，因为入册界面的素材一律走 S() 包装的 BASE_URL、新加的 state 与 effect 都在使用点之前。",
      "③【顺带·那一行三个数字全都过期了】不只文件数。按实际数：210 个 js/jsx（不是 203）、其中 29 份测试（文档写 30）、非测试代码 46157 行约 4.62 万（文档写 4.26 万）。测试份数与行数这两个数字守卫并不检查，所以能一直漂着没人发现。三个都按实测改准了；如果你数测试的方式跟我不同（我算的是文件名含 .test. 的），以你的为准。",
      "验证：本地把 docsTree 的三条文件系统守卫全部模拟跑过（登记 0 缺、幽灵 0 条、声明数 210 = 实际 210），version.js 三条结构守卫也过。vitest 全量仍需 CI 或本地 npm run verify 确认。",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      "起因：docs 最后同步是 0724~0727，而代码到了 0730——中间隔了三个门派、雅江据点、家园拆房、四份词典合并、地图东西向订正。先 grep 出「文档说的」与「代码现状」对不上的地方，再逐处改，不做无差别重写。",
      "①【终极0727文件数关系.md·标为历史快照】它是 58KB 的架构叙事，不只是文件清单，逐行更新不现实也没必要。头部加权威分工表（文件清单看文件树、依赖形态看瘦身方案、每轮改动看 version.js、0727 前的架构叙事看它自己——最后这部分不会过期）。文末新增「0728~0730 增补」只记结构性变化：四个新增单一出口、外层东西向订正、内层坐标重排、地上物改房间级、被动层接线、三门派落地。",
      "②【武器物品系统说明.md / 攻击力计算公式.md·bonus 已不存在】两份都还在描述饰品的 bonus（白0.3→红6），而它 0730 已从全项目删除。就地改成「战力全在 sixDim 与 effect 上」，并写清为什么删（不接任何战斗逻辑，纯装饰性数值）。",
      "③【README.md·权威链补两条带自动校验的】原权威链只有四级，现补入 version.js、文件树、瘦身方案，并点明文件树与瘦身方案是**唯一带测试守卫**的两份（docsTree.test.js 检查登记完整性/幽灵条目/文件数一致，抓到过好几次「删了模块忘了删登记」）。",
      "④【README.md·过期点从 6 条补到 11 条】新增：终极0727 已标历史快照、饰品 bonus 已删、外层地图已改东西向（旧文档写「往西出乡」的一律作废）、moveExplainer.js 已删除、「148 招」是旧数字（现 246 招）。",
      "⑤【项目状态.md·加当前快照表】正文停在 0721~0724，逐节重写不现实。头部加一张 0724→0730 对照表：文件 167→203、非测试行数 3.65万→4.26万、测试 46→643、门派 4→9、外层据点 17→18、家园从四间大屋拆成 14 间真房间。并写明这三天的主线不是加功能，是把「同一件事散在多处」收成一处。",
      "验证：npm run verify 通过（vitest 643/643 + pages 构建）。",
    ],
  },
  {
    codename: "独孤九剑全图无处可学——令狐冲墓是拜祭殿不是授业处，改通并新增「参悟」路径",
    time: "2026-07-30 03:20",
    notes: [
      "①【根因·类型挂错，skillSet 没人读】令狐冲墓（dugu_tomb）挂着 skillSet:\"独孤\"，但它的 type 是 **BUILDING_TYPE.TEMPLE**——渲染的是拜祭殿（求 karma），CenterPanel 的 TEMPLE 分支压根不读 skillSet。于是建筑描述里白纸黑字写着「墓壁刻有独孤九剑剑诀，外功≥80可参悟习得」，这句话从来没有落地过：两门独孤武学（独孤九剑·橙、剑冢独坐·橙）**在全图无处可学**。又是「数据写了但没接线」，而且这次连门都进不去。",
      "②【为什么不是简单改成武馆就完事】这两门 price 都是 0。项目里 price 0 的既定含义是「不外卖」（梯云纵藏经阁宝箱、一字电剑练功堂秘传、血刀掉落、三星试炼），武馆界面对它们只会显示「0 两 · 购买」，语义不对。所以补了一条**参悟**路径。",
      "③【新增 insight 字段】武学可写 `insight: { stat, threshold, label }`。WuguanScreen 见到它就把「N 两 / 购买」换成「参悟 · 外功≥80」，够不到时不给按钮、直接说还差多少：「壁上剑痕看得见，看不懂——外功 45/80」。独孤九剑 80、剑冢独坐 60（先坐得住，才看得懂）。",
      "④【门槛在服务端也挡一道】handleBuySkill 里对 insight 类走独立分支：不扣钱、但要校验属性够不够。界面按不出来不等于调用不到，门槛只写在界面上等于没写。习得叙事也换成参悟的口吻，不再是「购得秘籍，费银0两」。",
      "⑤【新增 5 条守卫】令狐冲墓必须是授业类建筑（否则 skillSet 又没人读）；独孤两门必须不标价且带门槛；独孤九剑门槛必须高于剑冢独坐；insight 的 stat 必须是角色身上真有的字段（写错就永远取到 undefined、判定恒假）；标价与参悟两条路径互斥（既标价又要参悟，界面会自相矛盾）。",
      "验证：npm run verify 通过（vitest 643/643 + pages 构建）。",
    ],
  },
  {
    codename: "更正上一条的夸大 + 补重复键守卫；顺带记下霜天月的门槛是外功满值",
    time: "2026-07-30 02:40",
    notes: [
      "①【更正·我上一条把话说大了】上一轮说「挖出两把永远刷不出的红档神兵」，实际只有**一把**。逐条核过：坏键「雪山派·雪山崖底」确实埋着红档 **霜天月**（呼延大侠佩剑），这个属实；但另一条坏键「贡措海·令狐冲墓」里只有一件白档念想物「锈透的长剑」（「拔出来剑身就碎了。是个念想，不是兵器」），**不是神兵**。**地藏**（桑杰大侠的刀）一直好好挂在「贡措海·贡措海密室」，那个房间存在、键也没错，从没坏过。哑 bug 是真的，规模是一把不是两把。version.js 与 groundItems.js 的注释都已改正。",
      "②【新增·重复键守卫】改键名时最容易踩的坑：把 A 改成 B，而 B 已经存在——JS 对象字面量里后写的那份会把先写的**整个静默覆盖**，不报错不警告，只是一批地上物凭空消失。上一轮把「令狐冲墓」改成「前辈墓地」时就差点撞上已有的「贡措海·前辈墓地」（那条本来就在，装着锈透的长剑）。运行时对象已经看不出重复，所以这条守卫走源码正则查。",
      "③【记一笔·霜天月的门槛是外功满值】雪山崖底的解锁条件是 `{ stat: waigong, threshold: 100 }`，而外功上限就是 100（面板显示 `外功 X/100`）。也就是说这把剑要求**外功练满**才进得去，是全游戏最硬的一道门槛。设计上大概是有意的（红档神兵配终极门槛），但此前那条键是坏的，没有任何玩家可能走到这一步、从而发现门槛这么高——现在键修好了，这个数值值不值得回调请你过一眼。",
      "验证：npm run verify 通过（vitest 638/638 + pages 构建）。",
    ],
  },
  {
    codename: "地上物改房间级（顺手挖出两把永远刷不出的红档神兵）+ 家园入口挪进左栏建筑区",
    time: "2026-07-30 02:00",
    notes: [
      "①【此地之物本该属于某一间屋子】GROUND_ITEMS 此前是**据点级**：锦官城 3 件地上物在它 31 间房每一间都躺着，鱼定村 2 件铺满 19 间，雅江 3 件铺满 15 间——同一块碎石你在城门看见一次、进当铺又看见一次、上钱庄还看见一次。55 件全部并入 GROUND_ITEMS_INNER 的「据点·房间」键，注入逻辑不再按 room.name 取。据点级表清空并保留空对象，只为不炸旧 import——新增地上物从此写不出据点级。",
      "②【暂落 anchor，后续可细分】55 件目前都归到各据点的 anchor（到达即见的那间）。这一步只解决「铺满全据点」，不做逐件重新归位——像「驿站旧马掌」该挪去马行、「半卷税赋黄册」该挪去账房，那是后续可以慢慢调的，键的形状已经支持了。",
      "③【挖出一把永远刷不出的红档神兵】加「键的两截必须真实存在」守卫时抓到两条死键：「雪山派·雪山崖底」——雪山崖底其实在**顶峰**不在雪山派；「贡措海·令狐冲墓」——该房间早已并入**前辈墓地**。⚠ **更正**：当时我说这两条各埋着一把红档，说大了。实际只有前者藏着红档 **霜天月**（呼延大侠的佩剑）；后者那条键里只有一件白档念想物「锈透的长剑」。**地藏**（桑杰大侠的刀）一直好好挂在「贡措海·贡措海密室」，从没坏过。哑 bug 是真的（不报错、东西不出现），但规模是一把不是两把。",
      "④【🏠 家园入口从顶栏挪进左栏「此地建筑」】家园设施本来就是「这间屋子里有什么可用的」，跟顶栏那排全局按钮（设置/存档/图鉴/音乐）不是一回事，摆在顶栏既占地方又容易被当成全局功能。现在跟建筑并排列在左栏，还顺带把该屋能用的设施名列在后面（木箱·烹饪台·酒窖·菜畦）。",
      "⑤【新增 3 条守卫】据点级表必须为空；每个键都是「据点·房间」且两截都真实存在（就是它抓出的③）；每条地上物都要有名字与描述（左栏那行不能空白）。",
      "验证：npm run verify 通过（vitest 637/637 + pages 构建）。",
    ],
  },
  {
    codename: "建筑面板只占半屏，下半截看不到——改为独占整条主叙事栏",
    time: "2026-07-30 01:05",
    notes: [
      "①【根因·两个 flex:1 的兄弟在平分高度】CenterPanel 里「建筑/交易内联功能区」与下方「叙事正文区」是并排的兄弟节点，两个都写 flex:1。flex 布局下这就是平分剩余高度——建筑面板永远只拿到一半。",
      "②【后果】铁匠铺、商店、武馆、拍卖行这些内容长的面板，下半截直接被截掉：底下的按钮看不见，也滚不到——外层容器不滚，面板自己那层 overflowY 又被压在半屏里，滚动条量程只有半屏那么长。代码注释里当时还写着「与下方叙事区平分剩余高度，一直伸到底部行动栏」，前半句是实情，后半句是愿望。",
      "③【修法：让位，不是压缩】建筑面板展开时，叙事正文区整个 display:none 让出整栏，建筑面板独占。压缩叙事区（比如给它 flex:0.3）解决不了「只有一半」这个根因，只是把一半变成七成，内容再长一点照样截。想看叙事就点「▾ 最小化」，叙事区立刻恢复全高——这条退路本来就有，现在它才真正有意义。",
      "④【顺带确认里层没有二次封顶】逐个查了 buildings/ 下的高度限制：GambleStoneScreen / InnScreen / HomesteadPanel 那几处 maxHeight 都是全屏浮层自己的，JadeShopScreen / AuctionScreen 的 maxHeight:200~260 是内部小列表的自滚区，都不影响外层撑满。",
      "⑤【新增 3 条源码级守卫】CSS 在 node 里没法真渲染，退一步盯住关键条件：叙事正文区必须带着「建筑展开时让位」的 display 条件；建筑功能区容器必须是 flex:1 + minHeight:0 + 自滚（少了 minHeight:0，flex 子项不收缩，内部滚动条根本出不来）；最小化入口必须还在——让位之后它是唯一能看回叙事的路。",
      "验证：npm run verify 通过（vitest 634/634 + pages 构建）。⚠ 我无法在浏览器里实测渲染，这条改动请你开一间铁匠铺或商店看一眼是不是拉到底了。",
    ],
  },
  {
    codename: "图鉴只显示 17/246 招——四个招式源归一，补 ESC 关闭",
    time: "2026-07-30 00:20",
    notes: [
      "①【我上次的数字是旧的，而且合并没做到位】此前报的「148 招」是很久以前 SKILL_CATALOG 只有 10 门、专属招 116 个时候的数。这几轮加了峨眉11/唐门2/血刀1/三星2/回气听桥20，又新增七位具名人物的专属招，真实数字早就不是那个了。更要紧的是：上一轮我只合并了「标志位→人话」的**描述函数**，四个**招式数据源本身**根本没合并——这才是没到位的地方。",
      "②【实际家底】武馆 SKILL_CATALOG 56 门／专属 NPC_SIGNATURE_MOVES 168 招（40 人）／制式 MOVE_POOL 18／博弈 BURDEN_MOVE_POOL 4，合计 **246 招**。",
      "③【图鉴只显示 17 招】CodexScreen 里有张 SKILL_GROUP_LABEL 只列了玉泉/雪山/锦官，而筛选写的是 `.filter(([k]) => SKILL_GROUP_LABEL[k])`——**把展示名当成了白名单**。独孤/青城/峨眉/唐门/血刀/三星六家共 39 门，连同全部 168 个专属招、18 个制式招、4 个博弈招，在图鉴里一个都看不到，而且丢得毫无痕迹（不是报错，是静默过滤）。又一次白名单模式。",
      "④【新增 kungfu/moveCodex.js·唯一读取口】把四个源归一成同一种形状（与 move 对象兼容，直接喂得进 MOVE_RULES）。专属招只写了 archetype+名字+描述，数值过 resolveArchetype 按主人档位现算；主人的档位取自 RESIDENT_NPCS 的 levelCap，**不在专属招表里另存一份 tier**——存两份必漂移，这个项目已经在别处栽过。分组名查不到时用 key 兜底，绝不丢条目。",
      "⑤【顺带补 npcSignatureMoves 的 SIGNATURE_SLOT_KEYS】此前没有权威槽位清单，需要枚举槽位的地方各自裸用 Object.keys，记录上多出 unlearnable 这类元数据键就会被当成一招算进去。",
      "⑥【图鉴改版】武学页加来源分页（武馆56／专属168／制式18／博弈4），顶部显示「全图共 246 招 · 本页 N 招」；条目行补主人名与槽位、内功轻功标注、不可学标记；搜索同时匹配招名/描述/主人名。",
      "⑦【ESC 关闭】玩家把字号调大之后图鉴内容会顶出屏幕高度，右上角那个叉点不到，除了刷新页面没有别的退路。这条很早就提过，一直拖到现在才补上，叉号 title 也标了「也可按 ESC」。",
      "⑧【新增 7 条守卫】四个源一个都不许少；SKILL_CATALOG 每一门都必须在总录里（防再被分组名白名单吃掉）；每个武学集都要有分组；专属招全部收录且都挂主人；档位取自 levelCap 不另存；每条都吐得出人话或至少有描述（防图鉴一片空白）；key 唯一。",
      "验证：npm run verify 通过（vitest 631/631 + pages 构建）。",
    ],
  },
  {
