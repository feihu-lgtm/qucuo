...
export const VERSION_HISTORY = [
  ...
  },
  {
    ...
    notes: "玩家反馈：私聊旁白日志一下出两条、且连发时两条聊天记录一样。查明两个成因：①并发——talkToNarrator 设计成不 await、允许并行，玩家一条没返回就发第二条，两条都读到发送前同一份 convo(历史一样)，且流式占位用的 logIdx.current 是共享 ref，两条流式回调互相踩踏，表现为日志出两条/重复。②act 的 whisper 分支自己 startTrace+endTrace 了一条空壳\"私聊已发送\"，talkToNarrator 内部又开一条完整 trace，全流程日志里每次私聊出现两条 trace。修法：①私聊串行——新增 talkBusyRef(同步 ref，不等 re-render，能挡住极快连点)，一条在途时挡住第二条实际发送，输入框仍可预打字(只拦发送不拦输入)；ref 在 talkToNarrator 的 CRASHED分支/正常结尾/catch 所有出口重置，确保不永久锁死；act 依赖数组补 pendingTalks。②删掉 whisper 分支的空壳 trace。③输入框 placeholder 私聊态改为\"旁白回话中…可先打字，待她说完再发\"(此前误写\"可继续发\")，对话模式不受影响仍可并发。esbuild + vite.config.pages.js 完整 build 通过。",
  },
  {
  ...
  },
  {
    ...
    notes: "接着一期图鉴做完二三期。【二期·分布】每件物品新增一行\"📍在哪获得\"：读 distribution.js 的 ITEM_DISTRIBUTION(物品名→{regions,shopTypes})——在表里(可上架货，346件中271件)译成\"<地域>(据点)的<店类>有售\"，地域用 regionMap.js 的 DISTRICT_REGION 反查落到具体据点(如山野→黑风寨/大草甸/熊山)；不在表里(75件神兵/传说/剧情信物/任务物，如霜角/界石下的土/半块银锁片)标为\"奇遇·剧情或支线获得，寻常货架无售\"。命中率实测78%健康，不是大面积误判。武学的\"在哪学\"本就靠武馆分组标题(玉泉/雪山/锦官)表达，不再重复。【三期·搜索+收集进度】①两页签共用搜索框，按名字或介绍实时过滤(含清空按钮)；②\"只看已得\"开关；③组件新增 inv/skills 两个 props(MudRPG 传入)，用 Set 比对算出已拥有的物品名/已学武功名，每条命中的：玉石去灰度高亮+右下角✓角标+名字后\"已获得/已习得\"绿标；物品页顶显示\"已收录 N/346\"收集进度。未获得的玉石做 grayscale 灰化，一眼区分收没收集到。全部纯读取比对，不改任何状态。esbuild + vite.config.pages.js 完整 build 均通过；node 脚本验证了分布表命中率。至此图鉴三期全部完成。",
  },
  {
  ...
  },
  {
    ...
    notes: "两件。【A·体貌】此前 AI 手上只有名字、性别和七维数字，写到\"他伸手\"\"她抬头\"这类具体动作全靠现编，同一个人这轮精瘦少年、下轮魁梧大汉。新增 bodyProfile.js：12 个字段，每个带「定义」一起发（写法参考 Ji-Haitang/char_card_1 的 <UserInfo>——给模型的不是一个光秃秃的名词，而是「这一栏在描述什么」+「当前是什么样」）。①【两层蓝绿灯】公开层(身量/体型/面容/肤色/发式/声音/惯常穿着)按场合点灯：full/talk 这类近距离互动轮亮、move/settle 赶路结算轮灭，另开关键词通道(照镜/打量/衣着/长什么样等 23 词)让玩家主动提起时哪怕在赶路轮也亮；私密层(疤痕/体味/身体细节/敏感处/习惯癖好)**只认 ■ 模式**，硬条件，关键词点不亮、场合也点不亮。灭灯不只是省 token——赶路轮塞一段私处描写，模型真的会顺着那个方向写。留空的栏一个字都不发。②【面板】入口做在姓名右侧(◈体貌 N，N=已填栏数)。「不完全显示文本」的落法：列表默认只露每栏前 14 字、点条目才展开成输入框；私密层更进一步，默认整条打码成 ••••，要先点「显示」才看得见——这个面板是会在别人面前打开的。③【按体貌荐装】面板底部一个按钮，拿体貌去货架上比一遍挑三件。分工照 catalog.js 顶部那条老规矩：AI 只负责\"从这张清单里挑哪三件\"，数值一概不由它给——挑完拿名字回 CATALOG_INDEX 查真值，AI 报的名字对不上货架就整条丢弃(不做模糊匹配，\"看起来像\"一旦错了玩家拿到的就是另一件东西的数值)。回三件的名称/类别/品阶/描述/真实加成，**不给获取途径**。④注入点：主叙事 sys 末尾 + 私聊 sys，私聊时公开层常亮(她一直看着你)。【B·旁白聊任务线】全任务表二十几条线、每条四五个 stage，全量注入等于每次闲聊背一本攻略书，还会把玩家没接的线一起剧透。改成\"报全名才查\"：新增 gateQuestTopic(scanText, affection, quests, progress)——玩家说出任务标题(或标题里 ≥3 字的分段，如「师母之情」)才注入**那一条线**的完整 stage，含当前走到第几节(✓已过/▶当前/○未到)与每节的 playerHint；只泛泛问\"我该干嘛\"则只给一句话让她反问、要玩家报全名，不自己猜也不列任务名单。好感度门槛 30(QUEST_TALK_MIN_AFFECTION)：低于此数只给一条\"你俩还没熟到这份上\"的口径，不泄露任何步骤/人名/地点，且不许她解释这是好感度门槛。两条红绿灯的点灯明细都进 trace。vite build 通过；新增 38 条断言覆盖体貌两层的场合/关键词/■模式各组合、空栏不发、预览截断与打码、荐装请求构造，以及任务线的好感门槛(9/30/50)、报全名只出一条、分段别名命中、泛问不泄露、进度标注四态，全部通过。dev server 实机走完开局→游戏内，验证了体貌入口、编辑展开、折叠截断预览、私密层打码与揭码，干净标签页无控制台报错。荐装按钮需真实 API key 才能端到端跑，未验证模型侧。",
  },
  {
    codename: "修双调用模式五处：记忆链整条断了、截断不检测、新面孔人设退化、涌现失效、日志看不到prompt",
    time: "2026-07-24 18:05",
    notes: "上一轮修的是双调用的60秒超时整轮回滚，修对了，但双调用分支还欠着五处，本轮一次补齐。①【最严重·记忆链整条断了】extractionEngine 的 6 个意图 spec 没有一个产出 memory 字段，而双调用里 p = extracted?.p || {}，于是 p.memory 恒为 undefined，MudRPG 里三处 if (p.memory) 全部静默跳过——向量小纸条不写、当日原料 dayMaterialRef 不进(日总结没素材)、事实账本 registerFact 不登记。而事实账本正是旁白「全知事实」的来源(allFactSummaries)，账本空了她私聊时就真的什么都不知道。也就是说开了双调用 = S3小纸条/S4事实账本/日总结三条全哑，convo 还在所以短期上下文没事，断的是长期记忆。②【同源】mentionedNewNpcs 同样没有 spec 产出，NPC涌现第一阶段(传闻中的人物)在双调用下永不触发。①②修法：不在 6 份 schema 里各抄一遍，新增 COMMON_EXTRACT_TAIL 统一拼在每个意图 user prompt 末尾，明确要求在同一个顶层 JSON 里补 memory(≤50字客观事实，琐事可省)与 mentionedNewNpcs(被提到但不在场的新具名人物，没有可省)。另新增导出的纯函数 normalizeExtractedFields(parsed) 做规整——提取层通常是小/快模型，格式服从度不如主模型，memory 可能吐成对象或数字、mentionedNewNpcs 可能吐成\"甲、乙\"逗号串或单个字符串，而下游 writeNote/registerFact/recordRumoredNpcs 都假定 string 与 string[]；统一收成 string / 去重后的 string[]，空值直接删键而非留空串(下游一律 if (p.memory) 判断)。脏数据一旦写进事实账本，日后再查是谁写的很麻烦，所以拦在入口。③【新面孔人设退化】MudRPG 里 narrativeText 写死 (p.output||[]).join('')，但双调用的叙事在 rawFull 散文里、p.output 恒为空，于是喂给 mapDescriptionToGenParams 的只剩 brief，退回吃 luck 兜底——正是那段代码上方注释里说要修的\"猎户和商贩长出同一副筋骨\"，在双调用下静默失效。改为在两个分支各自赋值的统一变量(单调用=p.output拼接，双调用=rawFull)，传闻人物 recordRumoredNpcs 的 context 同一处修正。④【截断不检测】双调用只取 r.rawFull、把 finishReason 整个丢了，撞 length 上限时既不提示也不重说。现在接住并判定，命中时 trace 记 block 态 + 日志明示\"提取层只能按这段残文结算状态\"。不自动重说：叙事已经打到屏幕上(流式更是已转永久条目)，重说会把同一段剧情印两次；位置放在叙事落日志之后，否则\"以上是已收到的部分\"会排在正文上面。⑤【日志看不到prompt】单调用有 attachPipeline，双调用一处都没有(上一轮补了 traceStep 但没补这个)，出问题仍看不到喂进去的是什么。补在主叙事成功后、提取调用发起之前——getPipelineLog()[0] 取最近一条，等提取发完再挂就变成提取那条了。vite build 通过；新增 17 条断言覆盖 normalizeExtractedFields 的 memory 类型转换/空值删键、mentionedNewNpcs 的数组原样/顿号串/英文逗号串/单字符串/去重/去空项/空数组/非法类型，以及不误伤其他字段与 null 入参，全部通过。dev server 干净标签页复验无控制台报错。",
  },
  {
    codename: "好感度五档统一分界+攻略表点灯+篇幅100→500解锁；身世定为无名之人、旧信改赌石邀帖，配私聊话题红绿灯",
    time: "2026-07-24 17:43",
    notes: "两块。【A·好感度五档收口】①此前五档分界线在四个地方各写一遍(narratorVoicePrompt 的 if/else、NARRATOR_WHISPER_CONTEXT 的攻略表、affectionLabel、旁白Tab 的标签数组)，改一处漏三处。现在收成 narrator.js 的唯一一份 AFFECTION_TIERS[{key,min,label,words}]，配 affectionTier(affection) 查询函数，affectionLabel/旁白Tab/篇幅/攻略表全部改读它。②【攻略表点灯】NARRATOR_WHISPER_CONTEXT 原本把五档攻略规则全文一次性塞进 prompt，模型每次都要读另外四档跟当前无关的规则——既烧上下文又容易串档(好感41 读到「≥90 问什么答什么」照样会心软)。常量改成函数 buildNarratorWhisperContext(affection)，只点亮当前所在那一档，同 worldbook 蓝绿灯思路。③【篇幅分档解锁】上一版的 narratorWhisperWordCount 单值(300)改成 narratorWhisperWords 五档对象，出厂 100/200/300/400/500——冷淡期她本就没什么话，篇幅跟着好感度一起解锁，与文风、攻略档位同一条曲线。loadConfig 加逐键补全，旁白Tab 的单滑块换成五档数字框、当前档位用 ▶ 高亮。【B·身世与旧信】④身世此前是空的，玩家一问私聊旁白就只能现编，这局说是将门遗孤下局说是灭门孤儿。现在定死：主角就是无名之人，没有失落的血脉、没有隐藏的仇家、没有门派遗孤身份等着揭晓——这是立意不是留白(他挣到的每一分都是自己挣的)。⑤旧信从「无落款的旧信」改为「赌石邀帖」：天都镇玉器轩温掌柜托人捎出，请他去后院料场看开石，落款写明是温掌柜本人，措辞是生意人的客气话。物品 desc、开场白(presets/qucuo.js opening 加一句点明无姓氏无师门)、OpeningSequence 两张图的 caption、契诃夫之枪第1条全部同步改写，并在契诃夫之枪里明确写上「不许把这张帖子写成血统悬疑或身世伏笔」。⑥【私聊话题红绿灯】新增 narrator.js 的 WHISPER_TOPICS + gateWhisperTopics(scanText)：纯关键词匹配，扫描「玩家这句话+上一条回复」，命中才注入，没命中一个字不发。与 gateScenario 的区别是方向相反——gateScenario 是「默认全给、按条目灭灯」(减法)，这里是「默认不给、按命中点灯」(加法)，因为身世/邀帖这类冷门追问绝大多数轮次用不上，常驻纯属白烧。目前两条：origin(身世，keys 含 身世/我是谁/来历/父母/爹娘/血脉/遗孤 等 20 词，含繁体)、letter(赌石邀帖，keys 含 信/邀帖/落款/赌石/温掌柜/天都镇/玉器轩 等)。点灯明细进 trace「私聊话题·红绿灯」。worldbook.js 的契诃夫之枪条目 keys 也补上 邀帖/赌石/温掌柜/来历/无名。vite build 通过；另写了 24 条纯函数断言覆盖档位分界(0/19/20/44/45/69/70/89/90/100)、篇幅解锁与自定义覆盖、攻略表只出当前档不带另外四档、话题红绿灯的命中/未命中/双命中/空输入，全部通过；dev server 实机确认五档篇幅面板渲染与 ▶ 跟随好感度联动正常。",
  },
  {
    codename: "新增「旁白」设置Tab + 私聊接入世界书蓝绿灯 + 截断检查与前缀累积修复",
    time: "2026-07-24 17:52",
    notes: "承接上一版，把旁白线剩下的三件事一次做完。①【旁白Tab】落地 docs/旁白系统_黑客帝国支线设计.md §八.1 的\"旁白独立设置Tab\"：设置主页从四卡变五卡，新增🎭旁白。内含好感度(滑块+数字+五档标签一键跳档，标签按 <20/<45/<70/<90/≥90 与 narratorVoicePrompt 同一套分界)、剧情阶段强制切换(第一幕暧昧/第二幕告白后/第三幕宕机，宕机在游戏内不可逆、从这里能掰回来；切到告白后额外露出记忆碎片计数0-8)、私聊篇幅目标字数、私聊token输出上限(与API配置里那项是同一个 callTokenLimits.narratorWhisper)、以及新增的\"旁白专属世界书\"。SettingsPanel 新增 narrator/setNarrator 两个 prop，MudRPG 的两处 <SettingsPanel> 调用点同步传入。②【旁白专属世界书】新增 cfg.narratorLorebook(默认空)，内容只拼进私聊 system prompt、不进主叙事——主叙事的世界观走预设/scenario 那一套，这里写的是\"只有她自己知道的事\"。留空则一个字不发；有内容时 trace 记一条\"旁白专属世界书·注入N字\"。③【私聊过蓝绿灯】私聊此前直接拼 preset.scenario 全文，每次闲聊都要背一遍地图拓扑、契诃夫之枪、路途遭遇、装备品质六档表——聊天一句用不上，纯烧上下文还把她往说书人腔调上拽。改为与 act 同一个 gateScenario，scope 走独立的 \"whisper\"(不在任何条目的 scopes 里)，因此专项段全靠关键词点亮：问路才亮拓扑、提到人名才亮具名人物、聊到掉落才亮装备规则。认不出标题的段落照旧常驻(蓝灯)，用户改过 scenario 或换自定义预设都不会丢内容。点灯明细写进 trace「世界书·总纲」，与 act 同款格式。④【截断检查】私聊此前把 finishReason 整个丢掉(主叙事那条路一直有判定+自动重说)，撞上限时界面毫无提示、只表现成\"她话说一半\"。现在流式/非流式两条分支都接住 finishReason，命中 length/max_tokens 时 trace 记 block 态、日志明确提示撞了多少 token 上限并指路设置→旁白。不做自动重试：私聊不消耗回合、玩家再问一句即可，重试反而多烧一次钱。附带修掉一个更隐蔽的危害——⟦好感X⟧要求写在整段最末尾，被截断就一定读不到，parseAffTag 于是兜底+1，她明明在敷衍系统却在加好感；现在 parseAffTag 多返一个 tagged 标志，截断且未读到标记时好感增量按\"未表态\"处理成 0。⑤【前缀累积】存 convo 时无条件加「（旁白私聊回应）」，模型看见历史里这个模式后自己也写一遍，于是逐轮累积成「（旁白私聊回应）（旁白私聊回应）…」(用户日志里从第4轮起已是双份)。新增 stripEchoPrefix 在存之前把模型自己写的前缀剥干净——前缀由系统负责加，模型写的一律不算数。vite build(--config vite.config.pages.js)通过，并在 dev server 上实机验证了旁白Tab的渲染、好感度档位高亮联动、阶段切换与记忆碎片条件显示，控制台无报错。",
  },
  {
    codename: "私聊旁白修\"每次只说半句\"：补篇幅指令(默认300字)+token上限600→6000可调",
    time: "2026-07-24 17:24",
    notes: "用户反馈私聊旁白每次只回几十字、还常断在逗号上。排查日志坐实是两个独立原因叠加，不是一个bug。①【真截断】talkToNarrator 的 maxTokens 写死 `Math.min(apiCfg.maxTokens, 600)`——这是上一轮\"9处写死maxTokens全抽成callTokenLimits\"漏网的第10处(那轮改了questHarness/pigeonReply/inspect/persuasion/extraction×2/knowledge/direction/连接测试，唯独没动私聊)。600这个值在带思考的模型下会被思考token吃穿，正文写二三十字就撞length上限。日志证据是⟦好感X⟧标记：该标记被要求写在整段最末尾，耗时4.7s的那次完整带标记，耗时8~12s的三次全部缺标记且全部断在半句——缺标记=没写到末尾=撞上限。附带危害：标记被截断后 parseAffTag 兜底给+1，于是她明明在敷衍玩家、系统却在加好感，好感度被静默篡改。修复：DEFAULT_CALL_TOKEN_LIMITS 新增 narratorWhisper:6000，调用点改读 cfg.callTokenLimits.narratorWhisper ?? 6000，SettingsPanel「各类调用·token输出上限」补一行「私聊旁白」(loadConfig 本就是逐键补全，老存档自动吃到新键)。②【根本原因·没截断也短】私聊的 system prompt 里压根没有篇幅指令——主叙事有 buildSysBase 的「篇幅要求：${lenNote}」硬指令(默认约900字)，私聊那条 sys 只拼了 WHISPER_CONTEXT+voice+worldState+facts+recall+scenario，一个字的长度要求都没有，模型没有目标就按聊天默认长度走。这条不修，token调到10万她照样只说两句。修复：narrator.js 新增 narratorWhisperLengthNote(words)，apiConfig 新增 narratorWhisperWordCount:300(私聊本就不该像叙事那么长，给独立短篇幅线)，拼在 sys 最末尾——沿用本项目已验证的酒馆插入深度经验(Depth 0=贴生成处=最强位)，与\"成文铁律放userContent末尾\"同一条道理，不埋进开头被当耳旁风。该指令保留两个逃生口：冷漠期(<20)对系统元问题的打断式回绝、以及CRASHED后的空壳应答，那两种本就该是一句话，不该被篇幅要求硬撑长。③需要说清的是：还有一部分\"短\"是设计如此不是bug——好感度41落在<45档，narratorVoicePrompt 该档原文就写着\"简短回应几句\"\"语气冷淡、公事公办\"，私聊分档攻略20-45档也写着\"勉强点一两句，含糊、留一半\"。养到45+才\"愿意闲聊\"。本轮只修掉技术性的短，人设性的短保持不动。vite build(--config vite.config.pages.js)验证通过。注：根目录的 vite.config.js 因 rollupOptions.input 引用了仓库里并不存在的 debug.html/debug-gamble.html/debug-item.html，`npm run build` 在干净检出上必然失败(与本轮改动无关的既有问题)，故用 pages 配置验证。",
  },
  {
